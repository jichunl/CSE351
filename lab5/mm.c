...
void* mm_malloc (size_t size) {
  ...
  // Search for the first free block with requested size
  ptrFreeBlock = searchFreeList(reqSize);

  // Check if the required block is found
  if (ptrFreeBlock == NULL) {
    // If no free block would fit
    // request more heap space with at least reqSize
    requestMoreSpace(reqSize);
    ptrFreeBlock = searchFreeList(reqSize);
  }

  // The required block is found, remove it from the free list
  removeFreeBlock(ptrFreeBlock);

  // Get the size of block
  blockSize = SIZE(ptrFreeBlock->sizeAndTags);
  
  // Check whether the block size is greater than MIN_BLOCK_SIZE plus reqSize
  if (blockSize - reqSize >= MIN_BLOCK_SIZE) {
    // The block needs to be split into a result block and remaining block
    // Set size and tags for result block
    precedingBlockUseTag = ptrFreeBlock->sizeAndTags & TAG_PRECEDING_USED;
    ptrFreeBlock->sizeAndTags = (reqSize | precedingBlockUseTag) | TAG_USED;
    // Get the address of remaining block
    BlockInfo * remainingBlock = (BlockInfo*) UNSCALED_POINTER_ADD(ptrFreeBlock, reqSize);
    // Set the size and tags of the remaining block
    remainingBlock->sizeAndTags = (blockSize - reqSize) | TAG_PRECEDING_USED;
    // Set the size and tags of the remaining block
    *((size_t*) UNSCALED_POINTER_ADD(ptrFreeBlock, blockSize - WORD_SIZE)) = remainingBlock->sizeAndTags;
    // Insert the remaining block back to the free list
    insertFreeBlock(remainingBlock);
  } else {
    // The block does not need to be split
    // Set the used tag of this block
    ptrFreeBlock->sizeAndTags =  ptrFreeBlock->sizeAndTags | TAG_USED;
    // Get the following block and set its preceding block use tag
  ...
  // Return the pointer of usable space of the block
